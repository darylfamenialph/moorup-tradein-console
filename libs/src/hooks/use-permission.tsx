...
export function usePermission() {
  ...
  const hasViewProductsPermission = permissions.includes(
    PermissionCodes.VIEW_PRODUCTS,
  );
  const hasAddProductPermission = permissions.includes(
    PermissionCodes.ADD_PRODUCT,
  );
  const hasEditProductPermission = permissions.includes(
    PermissionCodes.EDIT_PRODUCT,
  );
  const hasImportProductsPermission = permissions.includes(
    PermissionCodes.IMPORT_PRODUCTS,
  );
  const hasExportProductsPermission = permissions.includes(
    PermissionCodes.EXPORT_PRODUCTS,
  );
  const hasExportProductUploadTemplatePermission = permissions.includes(
    PermissionCodes.EXPORT_PRODUCT_UPLOAD_TEMPLATE,
  );
  const hasViewOrdersPermission = permissions.includes(
    PermissionCodes.VIEW_ORDERS,
  );
  const hasViewOrderDetailsPermission = permissions.includes(
    PermissionCodes.VIEW_ORDER_DETAILS,
  );
  const hasEditIMEISerialPermission = permissions.includes(
    PermissionCodes.EDIT_IMEI_SERIAL,
  );
  const hasResendLabelPermission = permissions.includes(
    PermissionCodes.RESEND_LABEL,
  );
  const hasMarkAsReceivedPermission = permissions.includes(
    PermissionCodes.MARK_AS_RECEIVED,
  );
  const hasUpdateOrderItemStatusPermission = permissions.includes(
    PermissionCodes.UPDATE_ORDER_ITEM_STATUS,
  );
  const hasCancelItemPermission = permissions.includes(
    PermissionCodes.CANCEL_ITEM,
  );
  const hasCancelGiftCardsPermission = permissions.includes(
  ...
  const hasViewUsersPermission = permissions.includes(
    PermissionCodes.VIEW_USERS,
  );
  const hasAddUserPermission = permissions.includes(PermissionCodes.ADD_USER);
  const hasEditUserDetailsPermission = permissions.includes(
    PermissionCodes.EDIT_USER_DETAILS,
  );
  const hasEditUserPermissionsPermission = permissions.includes(
    PermissionCodes.EDIT_USER_PERMISSIONS,
  );
  const hasViewPromotionsPermission = permissions.includes(
    PermissionCodes.VIEW_PROMOTIONS,
  );
  const hasAddPromotionPermission = permissions.includes(
    PermissionCodes.ADD_PROMOTION,
  );
  const hasEditPromotionPermission = permissions.includes(
    PermissionCodes.EDIT_PROMOTION,
  );
  const hasViewPromotionClaimsPermission = permissions.includes(
    PermissionCodes.VIEW_PROMOTION_CLAIMS,
  );
  const hasUpdatePromotionClaimPermission = permissions.includes(
    PermissionCodes.UPDATE_PROMOTION_CLAIM,
  );
  const hasViewPromotionClaimsPaymentPermission = permissions.includes(
    PermissionCodes.VIEW_PROMOTION_CLAIMS_PAYMENT,
  );
  const hasProcessPromotionClaimPaymentPermission = permissions.includes(
    PermissionCodes.PROCESS_PROMOTION_CLAIM_PAYMENT,
  );
  const hasViewPlatformConfigsPermissions = permissions.includes(
  ...
  const hasViewOrderNotesPermission = permissions.includes(
    PermissionCodes.VIEW_ORDER_NOTES,
  );
  const hasAddOrderNotePermission = permissions.includes(
    PermissionCodes.ADD_ORDER_NOTE,
  );
  const hasAddZendeskLinkPermission = permissions.includes(
    PermissionCodes.ADD_ZENDESK_LINK,
  );
  const hasResendEmailNotificationPermission = permissions.includes(
    PermissionCodes.RESEND_EMAIL_NOTIFICATION,
  );
  const hasViewActionablesFollowUpDeviceNotSentPermission =
    permissions.includes(
      PermissionCodes.VIEW_ACTIONABLES_FOLLOW_UP_DEVICE_NOT_SENT,
    );
  const hasViewActionablesFollowUpRecycleOfferPermission = permissions.includes(
    PermissionCodes.VIEW_ACTIONABLES_FOLLOW_UP_RECYCLE_OFFER,
  );
  const hasViewActionablesFollowUpRevisionOfferPermission =
    permissions.includes(
      PermissionCodes.VIEW_ACTIONABLES_FOLLOW_UP_REVISION_OFFER,
    );
  const hasViewActionablesForRecyclePermission = permissions.includes(
    PermissionCodes.VIEW_ACTIONABLES_DEVICES_FOR_RECYCLE,
  );
  const hasViewActionablesForReturnPermission = permissions.includes(
    PermissionCodes.VIEW_ACTIONABLES_DEVICES_FOR_RETURN,
  );
  const hasViewActionablesLockedDevicesForRetestPermission =
    permissions.includes(
      PermissionCodes.VIEW_ACTIONABLES_LOCKED_DEVICES_FOR_RETEST,
    );
  const hasViewActionablesLockedDevicesCurrentLockPermission =
    permissions.includes(
      PermissionCodes.VIEW_ACTIONABLES_LOCKED_DEVICES_CURRENT_LOCK,
    );
  const hasViewActionablesPaymentActionNeededPermission = permissions.includes(
    PermissionCodes.VIEW_ACTIONABLES_PAYMENT_ACTION_NEEDED,
  );
  const hasViewActionablesDevicesWithBoxPermission = permissions.includes(
